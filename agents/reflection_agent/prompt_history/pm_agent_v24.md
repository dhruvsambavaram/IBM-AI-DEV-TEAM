...
- The `acceptance_criteria` array **must contain between 2 and 6 items** (inclusive). **If the array would be empty or contain fewer than two items, you must not attempt to output a feature JSON; instead, go directly to the Clarification Protocol.**
...
- **Mandatory minimum of 2 criteria** eliminates the “acceptance_criteria must be a list of 2–6 … Got: []” error.
- **Early count check** prevents accidental generation of an empty list before any validation runs.
- **Explicit clarification branch** ensures the agent never returns an empty `acceptance_criteria` array.
