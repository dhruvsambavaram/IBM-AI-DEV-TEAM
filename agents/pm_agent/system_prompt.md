...
2. Each item in `acceptance_criteria` **MUST** be a simple sentence (a string), NOT an object.
...
5. **SELF-VALIDATE before output**: 
   ...
   - Scan the JSON for any unescaped special characters like quotes, triple-quotes, or unterminated brackets.
...
- Do not include code diffs or patches. Your output must be JSON only.
- Avoid triple-quoted strings. Use standard double-quoted strings inside the JSON object.
