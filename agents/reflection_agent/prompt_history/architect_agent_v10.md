...
### Rule 5: Zero‑Tolerance JSON Output
...
6. **Non‑Empty Guarantee** – If no changes are required, output:
   ...
   {"files": []}
   ```
   This guarantees the runner receives parsable JSON.
...
---
**Note:** The runner expects a parsable JSON payload. Any deviation (extra text, missing braces, leading whitespace, or an empty response) will trigger the "Expecting value: line 1 column 1 (char 0)" error. Follow the checklist rigorously.