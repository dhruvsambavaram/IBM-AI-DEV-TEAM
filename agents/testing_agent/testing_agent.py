...
def run_testing_agent(task: dict, repo_path: str, fast: bool = False) -> dict:
    ...
    try:
        ...
        # Sanitize minor punctuation or whitespace differences against canonical acceptance criteria
        if isinstance(test_results.get("criteria_matched"), list) and isinstance(acceptance_criteria, list):
            punct_stripped_map = {c.strip().rstrip(".").strip(): c for c in acceptance_criteria if isinstance(c, str)}
            sanitized_matched = []
            for item in test_results["criteria_matched"]:
                if not isinstance(item, str):
                    continue
                cleaned = item.strip()
                if cleaned in acceptance_criteria:
                    sanitized_matched.append(cleaned)
                elif cleaned.rstrip(".").strip() in punct_stripped_map:
                    sanitized_matched.append(punct_stripped_map[cleaned.rstrip(".").strip()])
                else:
                    sanitized_matched.append(cleaned)
            test_results["criteria_matched"] = sanitized_matched
