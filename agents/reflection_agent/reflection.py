...
# ──────────────────────────────────────────────────────────────────────────────
# IMPROVEMENT 4: Targeted fix instructions by failure category
# ──────────────────────────────────────────────────────────────────────────────

_TARGETED_FIX_INSTRUCTIONS = {
    "empty_output": (
        "The agent is producing empty responses. The rewritten prompt MUST add explicit "
        "instructions to always produce a non-empty response, and to never respond with "
        "only whitespace or an empty JSON object."
    ),
    "json_parse_error": (
        "The agent is producing malformed JSON. The rewritten prompt MUST add: "
        "1) An explicit JSON schema example in the output rules section. "
        "2) A self-check instruction: 'Before submitting, verify your output is valid JSON "
        "by checking that all strings are quoted, all brackets are closed, and there are no "
        "trailing commas.' "
        "3) Remove any instruction that allows prose before or after the JSON."
    ),
    "validation_error": (
        "The agent's output passes JSON parsing but fails schema validation. The rewritten "
        "prompt MUST add a concrete VALID EXAMPLE of a complete correct output, and an "
        "explicit list of what fields are required and what their types must be."
    ),
    "blocked_by_review": (
        "The agent's code is being rejected by the security review. The rewritten prompt "
        "MUST add a security pre-check section: 'Before generating any diff, verify: "
        "no hardcoded secrets, no SQL string concatenation, all user inputs are validated, "
        "no files outside scoped_files are touched.'"
    ),
    "rate_limit": (
        "The agent is hitting Groq API rate limits. The rewritten prompt MUST instruct "
        "the agent to produce more concise output — remove all explanatory prose from "
        "the output, output only the required JSON fields, use shorter variable names "
        "in code examples."
    ),
    "unknown": (
        "Review the failure summaries carefully and add targeted rules to the prompt "
        "to prevent each specific failure pattern from recurring."
    ),
}
...
def build_reflection_input(manager_report: dict, current_prompt: str, agent_name: str = None) -> str:
    ...
    IMPROVEMENT 4: Includes targeted fix instructions based on failure categories.
    ...
    # IMPROVEMENT 4: Build targeted fix instructions from failure_categories
    failure_categories = manager_report.get("failure_categories", {})
    agent_categories = failure_categories.get(agent_name, ["unknown"])
    targeted_instructions = []
    for category in agent_categories:
        instruction = _TARGETED_FIX_INSTRUCTIONS.get(category, _TARGETED_FIX_INSTRUCTIONS["unknown"])
        targeted_instructions.append(f"- [{category}]: {instruction}")
    targeted_section = "\n".join(targeted_instructions) if targeted_instructions else "- No specific category-based instructions."
...
Based on failure category analysis, apply these specific fixes:
{targeted_section}
...
Apply the TARGETED FIX INSTRUCTIONS above — each instruction corresponds to a specific
failure category detected by the Manager Agent.
...
def _call_watsonx(prompt: str) -> str:
    ...
    return call_bob_chat(prompt, max_tokens=650)
...
def compute_reflection_confidence(
    output: dict,
    original_prompt: str,
    failure_categories: list,
) -> float:
    """
    Compute a confidence score (0.0–1.0) for a reflection rewrite.

    Scoring rubric:
    - 0.3 points: change_summary has >= 3 bullet points
    - 0.3 points: change_summary total length >= 100 chars
    - 0.2 points: at least one failure_category-specific fix instruction
                   is present in the rewritten_prompt
    - 0.2 points: rewritten_prompt is longer than original_prompt

    Args:
        output: The reflection output dict with rewritten_prompt and change_summary.
        original_prompt: The original system_prompt.md content before rewrite.
        failure_categories: List of category strings (e.g. ["json_parse_error", "empty_output"]).
    ...
    rewritten = output.get("rewritten_prompt", "")

    # 0.3: change_summary has >= 3 bullet points
    if isinstance(change_summary, list) and len(change_summary) >= 3:
        score += 0.3

    # 0.3: change_summary total length >= 100 chars
    combined = " ".join(str(s) for s in change_summary) if isinstance(change_summary, list) else ""
    if len(combined) >= 100:
        score += 0.3

    # 0.2: at least one failure_category-specific fix instruction present in rewritten_prompt
    rewritten_lower = rewritten.lower()
    category_keywords = {
        "empty_output": ["non-empty", "never respond with", "empty"],
        "json_parse_error": ["valid json", "trailing commas", "brackets are closed"],
        "validation_error": ["valid example", "required fields", "schema"],
        "blocked_by_review": ["security", "hardcoded secrets", "sql"],
        "rate_limit": ["concise", "shorter", "prose"],
        "timeout": ["timeout", "time limit"],
    }
    if failure_categories:
        for cat in failure_categories:
            keywords = category_keywords.get(cat, [])
            if any(kw in rewritten_lower for kw in keywords):
                score += 0.2
                break  # only award once

    # 0.2: rewritten_prompt is longer than original
...
def run_reflection(manager_report: dict) -> list:
    ...
    IMPROVEMENT 5: Computes and stores a confidence score for each rewrite.

    Returns list of result dicts for the dashboard:
      [{ agent_name, old_version, new_version, change_summary, prompt_history_path, confidence }]
    """
    results = []
    # IMPROVEMENT 3 / Test Fix 3: Get failure_categories from manager_report
    failure_categories_map = manager_report.get("failure_categories", {})
    ...
    for agent_name in manager_report.get("underperformers", []):
        ...
        # Test Fix 3: failure_patterns is a list of strings (from extract_failure_patterns
        # in reflection.py), so pass directly. The manager's extract_failure_patterns returns
        # dicts, but the reflection's extract_failure_patterns returns strings.
        failure_patterns_for_validation = failure_patterns

        # Validate — log clearly if it fails so debugging is fast
        try:
            validate_reflection_output(output, expected_failure_patterns=failure_patterns_for_validation)
        ...
        # IMPROVEMENT 5: Compute confidence score
        agent_failure_cats = failure_categories_map.get(agent_name, ["unknown"])
        confidence = compute_reflection_confidence(output, current_prompt, agent_failure_cats)
        print(f"[reflection] Confidence score for {agent_name}: {confidence:.2f}")
