...
def test_update_stats_rolling_window_caps_at_ten():
    stats = initialize_empty_stats()
    ...
    # Window is 10 — all 7 are kept since 7 < 10
    assert stats["coding_agent"]["recent_outcomes"] == [True, True, False, False, True, True, True]
    assert stats["coding_agent"]["runs"] == 7
...
def test_extract_failure_patterns_from_review_findings():
    ...
    # IMPROVEMENT 3: patterns are now list of dicts
    pattern_strings = [p["pattern"] for p in patterns["coding_agent"]]
    assert "missing input validation" in pattern_strings
    assert "hardcoded secrets" in pattern_strings
