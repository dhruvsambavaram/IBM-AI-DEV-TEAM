...
from agents.reflection_agent.reflection import run_reflection, rollback_prompt_to_previous_version
...
def _emit_event(agent: str, event_type: str, data: dict | None = None, message: str = "") -> None:
    """Emit an unbuffered structured agent event to stdout for dashboard consumption."""
    event = {
        "event": "agent_event",
        "agent": agent,
        "type": event_type,  # "start", "progress", "done", "error", "retry"
        "message": message,
        "data": data or {},
        "timestamp": datetime.now(timezone.utc).isoformat(),
    }
    print(f"@@AGENT_EVENT@@{json.dumps(event, default=str)}", flush=True)
...
def _call_llm(prompt: str) -> str:
    ...
    try:
        return call_bob_chat(prompt, max_tokens=600)
    except Exception:
        return "{}"
...
def _parse_agent_json(raw: str) -> dict:
    ...
    try:
        ...
        if not isinstance(parsed, dict):
            raise ValueError(f"Agent returned JSON that is not a dict: {type(parsed)}")
        return parsed
    except (json.JSONDecodeError, ValueError) as exc:
        raise ValueError(
            f"Agent returned invalid JSON or non-dict. Error: {exc}. Raw (truncated): {raw[:300]!r}"
        ) from exc
...
def _append_run_history(manager_report: dict, reflection_results: list, agent_timings: list = None, auto_rolled_back: list = None) -> None:
    ...
    try:
        ...
        # IMPROVEMENT 6: Track auto-rolled-back agents
        if auto_rolled_back:
            entry["auto_rolled_back"] = auto_rolled_back
...
def run_pipeline(
    ...
    """
    os.environ["STREAM_AGENT_OUTPUT"] = "1"
    ...
    _emit_event("pm_agent", "start", message="Analyzing request and generating acceptance criteria...")
    _t0 = _time.time()
    try:
        os.environ["CURRENT_AGENT_NAME"] = "pm_agent"
    ...
    if task["status"] == "blocked":
        _emit_event(
            "pm_agent",
            "error",
            message="PM Agent blocked the request.",
            data={"acceptance_criteria": task.get("acceptance_criteria", [])},
        )
        ...
        _emit_event(
            ...
            message=f"Generated {len(task.get('acceptance_criteria', []))} acceptance criteria.",
        )
    ...
    for _arch_attempt in range(1, _ARCH_MAX_ATTEMPTS + 1):
        ...
        _emit_event(
            "architect_agent",
            "start",
            message=f"Inspecting repository structure and planning changes (attempt {_arch_attempt}/{_ARCH_MAX_ATTEMPTS})...",
        )
        _t0 = _time.time()
        ...
        try:
            os.environ["CURRENT_AGENT_NAME"] = "architect_agent"
        ...
        if task["status"] != "blocked":
            _emit_event(
                ...
                message=f"Scoped {len(task.get('scoped_files', []))} files.",
            )
        ...
        if _arch_attempt < _ARCH_MAX_ATTEMPTS:
            _emit_event(
                "architect_agent",
                "retry",
                message=f"Architect Agent failed attempt {_arch_attempt}. Retrying with feedback...",
            )
            ...
            _emit_event(
                "architect_agent",
                "error",
                message="Architect Agent blocked after all attempts.",
            )
    ...
    if not task.get("scoped_files"):
        ...
        _emit_event("architect_agent", "error", message="Architect Agent returned empty scoped_files.")
        task["status"] = "blocked"
        task["current_agent"] = "manager_agent"
        task.setdefault("history", []).append({
            "agent": "architect_agent",
    ...
    while True:
        ...
        _emit_event(
            "coding_agent",
            "start",
            message=f"Drafting unified git diff (attempt {task.get('retry_count', 0) + 1}/{MAX_RETRIES + 1})...",
        )
        ...
        def _llm_fn(prompt: str) -> str:  # noqa: E306
            ...
            raw = call_bob_chat(prompt, system_prompt=system_prompt, max_tokens=1500)
            ...
            if "@@" not in raw or not raw.startswith("---") or "<path>" in raw or not has_scoped_file:
                ...
                try:
                    recovery_raw = call_bob_chat(recovery_prompt, max_tokens=1500)
        ...
        try:
            _t0 = _time.time()
            os.environ["CURRENT_AGENT_NAME"] = "coding_agent"
            ...
            _emit_event("coding_agent", "error", message=f"Coding Agent runner error: {exc}")
            task["status"] = "blocked"
            task["current_agent"] = "manager_agent"
            task.setdefault("history", []).append({
                "agent": "coding_agent",
        ...
        if task["status"] == "blocked":
            _emit_event("coding_agent", "error", message="Coding Agent blocked.")
        ...
        if not preflight["passed"] or not preflight["syntax_clean"]:
            ...
            _emit_event(
                "coding_agent",
                "retry",
                message=f"Preflight AST syntax check failed: {err_msg[:120]}. Retrying...",
            )
            task.setdefault("history", []).append({
                "agent": "coding_agent",
            ...
            if retry_count < MAX_RETRIES:
                ...
                _emit_event("coding_agent", "error", message=f"Max retries reached on preflight failure.")
        ...
        _emit_event(
            "coding_agent",
            "done",
            data={"code_diff": task.get("code_diff", "")},
            message="Code diff generated and passed AST syntax check.",
        )

        print("[pipeline] Stage: Testing Agent")
        _emit_event("testing_agent", "start", message="Applying diff to test sandbox and executing pytest...")
        _t0 = _time.time()
        if fast:
            print("[pipeline] Fast testing enabled (baseline comparison skipped)")
        os.environ["CURRENT_AGENT_NAME"] = "testing_agent"
        ...
        if task["status"] == "blocked":
            _emit_event(
                "testing_agent",
                "error",
                data=task.get("test_results") or {},
                message="Testing Agent blocked.",
            )
        ...
        _emit_event("review_agent", "start", message="Evaluating code changes against security & quality standards...")
        _t0 = _time.time()
        os.environ["CURRENT_AGENT_NAME"] = "review_agent"
        ...
        if task["status"] == "blocked":
            _emit_event(
                "review_agent",
                "error",
                data=task.get("review_result") or {},
                message="Review Agent blocked.",
            )
        ...
        _emit_event(
            ...
            "done",
            data=task.get("review_result") or {},
            message=f"Review Agent status: {task.get('status')}",
        )
...
def _check_auto_rollback(stats: dict) -> list:
    """
    IMPROVEMENT 6: Auto-rollback check.
    Read the previous run from run_history.json. If an agent was reflected on
    in that run AND its current rate is lower than before AND below 0.4,
    roll back its prompt to the previous version.

    Returns a list of agent names that were rolled back.
    """
    rolled_back = []
    try:
        ...
        prev_run = history[-1]  # most recent completed run
        prev_reflections = prev_run.get("reflections_triggered", [])
        prev_rates = prev_run.get("agent_rates", {})

        for agent_name in prev_reflections:
            prev_rate = prev_rates.get(agent_name)
            curr_data = stats.get(agent_name, {})
            curr_rate = curr_data.get("rate")

            if prev_rate is None or curr_rate is None:
                continue

            # All 3 conditions: was reflected, rate is LOWER, and below 0.4
            if curr_rate < prev_rate and curr_rate < 0.4:
                success, target_v, msg = rollback_prompt_to_previous_version(agent_name)
                if success:
                    print(
                        f"[pipeline] AUTO-ROLLBACK: {agent_name} rate dropped from "
                        f"{prev_rate:.2f} to {curr_rate:.2f} after reflection "
                        f"— rolling back to v{target_v}"
                    )
                    rolled_back.append(agent_name)
                else:
                    print(f"[pipeline] AUTO-ROLLBACK attempted for {agent_name} but failed: {msg}")
    except Exception as e:
        print(f"[pipeline] AUTO-ROLLBACK check error: {e}")
...
def _finish(task: dict, agent_timings: list = None, username: str = "default") -> dict:
    ...
    _emit_event("manager_agent", "start", message="Updating agent statistics and evaluating performance...")
    stats = load_stats(STATS_PATH)
    stats = update_stats(task, stats)
    save_stats(stats, STATS_PATH)

    # IMPROVEMENT 6: Auto-rollback check before generating report
    auto_rolled_back = _check_auto_rollback(stats)
    ...
    if action == "rewrite_prompt":
        ...
        _emit_event("reflection_agent", "start", message=f"Rewriting prompt for underperforming agent(s): {underp}...")
        reflection_results = run_reflection(manager_report)
        task["reflection_results"] = reflection_results
        _emit_event("reflection_agent", "done", data={"results": reflection_results}, message=f"Prompt rewritten for {len(reflection_results)} agent(s).")
    ...
    _append_run_history(
        manager_report, reflection_results,
    ...
    _emit_event("manager_agent", "done", data={
        "status": task.get("status"),
        "consensus_report": task.get("consensus_report"),
        "manager_report": task.get("manager_report"),
    }, message=f"Pipeline run completed with status '{task.get('status')}' (Consensus: {c_pct}% - {c_lvl}).")
    return task
