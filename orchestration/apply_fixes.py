...
def load_approved_tasks(tasks_dir: str) -> list[dict]:
    """Return all tasks from *tasks_dir* (including user subdirectories) that are approved and have a non-null code_diff."""
    tasks = []
    ...
    try:
        ...
        if github_token and github_user:
            try:
                ...
                # If it's an HTTPS github URL, inject the credentials for this push only
                ...
                print(f"[apply_fixes] Warning: Failed to parse remote URL for authenticated push: {e}")
...
def save_task(task: dict, tasks_dir: str) -> None:
    """Write the updated *task* back to its JSON file."""
    path = task.pop("_file_path", None) or os.path.join(tasks_dir, f"{task['task_id']}.json")
